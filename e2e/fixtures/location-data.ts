...
export const sampleRows = Array.from({ length: 596 }, (_, sourceIndex) => {
...
export const columns = HEADERS.map((label) => ({ label, fieldKey: label, type: label.includes('Time') ? 'datetime' : 'string' }));
