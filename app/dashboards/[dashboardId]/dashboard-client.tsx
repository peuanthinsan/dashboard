...
const formatMonthYearLabel = (value: unknown) => {
  if (!value) return 'Unspecified';
...
export default function DashboardClient({
    ...
    return records
      .map((row: Record<string, unknown>, index: number): AlertRow | null => {
        ...
        return {
          ...
          dateTimeDisplay:
            formattedRows[index]?.[dateTimeColumn.field] ?? formatDateTimeLabel(dateValue),
  ...
  const remarkFleetSummary = useMemo(() => {
    ...
    const fleetList = Array.from(fleets).sort((a, b) =>
      a.localeCompare(b, undefined, { sensitivity: 'base' }),
    );
    const remarkList = Array.from(remarkTypes).sort((a, b) =>
      a.localeCompare(b, undefined, { sensitivity: 'base' }),
    );
  ...
  const alertsSortComparator = useMemo(
    () =>
      buildMultiSortComparator(alertsSorts, {
        dateTime: (row: { dateValue: Date }) => row.dateValue,
        vehicle: (row: { vehicle: string }) => row.vehicle,
        driver: (row: { driver: string | null }) => row.driver || '',
        alert: (row: { alertType: string }) => row.alertType,
        speed: (row: { speed: number | null }) => row.speed ?? 0,
        remarks: (row: { remarks: string | null }) => row.remarks || '',
        video: (row: { videoUrl: string | null }) => row.videoUrl || '',
      }),
    [alertsSorts],
  );
  ...
  const columnVisualStyles = useMemo(() => {
    const legend = palette.appEngineLegend || {};
    return {
      Date: {
        header: {
  ...
  const chartAccentColors = useMemo(() => {
    const legend = palette.appEngineLegend || {};
    return {
      line: legend.purple || palette.secondary.main,
      bar: legend.teal || palette.info.main,
    };
  }, [palette]);
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'stretch', md: 'center' }}>
        <Box>
          ...
          <Stack direction="row" spacing={1.5} sx={{ mt: 1, flexWrap: 'wrap' }}>
            {companyName ? (
              <Chip label={`Company: ${companyName}`} size="small" color="primary" variant="outlined" />
            ) : null}
            {organizationName ? (
              <Chip label={`Organization: ${organizationName}`} size="small" color="secondary" variant="outlined" />
            ) : null}
          </Stack>
          {lastUpdated && (
        ...
        <Stack direction="row" spacing={1} sx={{ flexShrink: 0 }}>
          <Button variant="outlined" href="/dashboards">
            Back to dashboards
          </Button>
          ...
          {sheetUrl ? (
            <Button variant="contained" href={sheetUrl} target="_blank" rel="noreferrer">
              View sheet
            </Button>
          ) : null}
        </Stack>
                          ...
                          {monthOverMonthSummary.rows.map((row) => {
                            ...
                            return (
                              <Paper
                                key={row.remark}
                                elevation={1}
                                sx={{ p: 2, borderRadius: 2, background: alpha(palette.primary.light, 0.08) }}
                              >
                          ...
                          <TableBody>
                            {paginatedAlerts.map((row) => (
                              <TableRow
                                ...
                                sx={{
                                  '&:nth-of-type(odd)': {
                                    backgroundColor: alpha(palette.primary.light, 0.06),
                                  },
                                  '&:hover': {
                                    backgroundColor: alpha(palette.primary.light, 0.14),
                                  },
