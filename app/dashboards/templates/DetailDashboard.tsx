...
const resolveRemark = (remark: unknown, alertType: unknown) => {
  ...
  const normalizedAlertType = normalizeLabel(alertType as string);
...
export default function DetailDashboard({ name, sheetId, gid }: DetailDashboardProps) {
    ...
    return records
      .map((row, index) => {
        ...
        return {
          ...
          driver: driverColumn ? (row[driverColumn.field] ? String(row[driverColumn.field]) : null) : null,
          alertType: String(alertType),
          speed: speedColumn ? row[speedColumn.field] ?? null : null,
          remarks: resolvedRemark,
          fleet: fleetColumn ? (row[fleetColumn.field] ? String(row[fleetColumn.field]) : null) : null,
          videoUrl: videoColumn ? (row[videoColumn.field] ? String(row[videoColumn.field]) : null) : null,
        };
      })
  ...
  const monthOverMonthSummary = useMemo(() => {
    ...
    const resolvedCurrentMonth =
      selectedMonthYear !== 'all' && months.includes(selectedMonthYear)
        ? selectedMonthYear
        : months[0] ?? null;
  ...
  const [alertsSorts, setAlertsSorts] = useState<SortConfig[]>([{ id: 'dateTime', direction: 'desc' }]);
  ...
  const alertsSortComparator = useMemo(
    () =>
      buildMultiSortComparator(alertsSorts, {
        ...
        speed: (row: AlertRow) => (typeof row.speed === 'number' ? row.speed : Number(row.speed ?? 0)),
  ...
  const getAlertsSortDirection = (columnId: string): SortDirection | false =>
  ...
  const columnVisualStyles = useMemo(() => {
    ...
    const headerStyles = (backgroundColor: string, color: string): SxProps<Theme> => ({
      backgroundColor,
      color,
    });
    return {
      Date: {
        header: headerStyles(legend.purple || palette.secondary.main, palette.getContrastText(legend.purple || palette.secondary.main)),
      },
      Vehicle: {
        header: headerStyles(legend.teal || palette.info.main, palette.getContrastText(legend.teal || palette.info.main)),
      },
      Alert: {
        header: headerStyles(legend.blue || palette.primary.main, palette.getContrastText(legend.blue || palette.primary.main)),
      },
      Speed: {
        header: headerStyles(legend.green || palette.success.main, palette.getContrastText(legend.green || palette.success.main)),
      },
    };
  }, [theme]);
                            ...
                            {paginatedAlerts.map((row) => (
                              ...
                              >
                                ...
                                <TableCell align="right">
                                  {row.speed != null ? Number(row.speed).toLocaleString() : '—'}
                                </TableCell>
