...
export default function LoadingState({ lang = 'th', message, detail }: LoadingStateProps) {
  return (
    <section
      className="rounded-3xl border border-fuchsia-200/70 bg-white/80 p-6 shadow-xl backdrop-blur dark:border-fuchsia-900/70 dark:bg-slate-900/70"
      role="status"
      aria-live="polite"
    >
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {detail ?? t(lang, 'Fetching the latest data and dashboard insights.', 'กำลังดึงข้อมูลล่าสุดและข้อมูลเชิงลึกของแดชบอร์ด')}
          </p>
        </div>
      </div>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={`loading-card-${index}`} className="flex h-28 flex-col justify-between rounded-2xl border border-fuchsia-200/70 bg-fuchsia-50/70 p-4 shadow-[inset_0_0_0_1px_rgba(148,163,184,0.1)] dark:border-fuchsia-900/70 dark:bg-slate-950/40">
