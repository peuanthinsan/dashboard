...
export default function DetailDashboard({
  ...
  const latestRemarkVideoTables = useMemo<RemarkVideoTable[]>(() => {
    ...
    return allowedRemarkTargets
      ...
      .filter((sample): sample is RemarkVideoTable => sample != null);
  }, [allowedRemarkTargets, filteredAlerts]);

  const totalLatestRemarkVideos = latestRemarkVideoTables.reduce((sum, table) => sum + table.rows.length, 0);
            ...
            <section className={dashboardSectionClass}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="text-sm text-slate-500 dark:text-slate-400">
                  {totalLatestRemarkVideos} {lang === 'th' ? 'วิดีโอ' : 'videos'}
                </span>
              </div>
              {latestRemarkVideoTables.length === 0 ? (
                ...
                <div className="mt-6 space-y-6">
                  {latestRemarkVideoTables.map((table) => (
                    <article
                      key={table.remarkType}
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <h3 className="text-base font-semibold text-slate-900 dark:text-white">{table.remarkType}</h3>
                        <span className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
                          {table.rows.length} / 10 {lang === 'th' ? 'วิดีโอ' : 'videos'}
                        </span>
                      </div>
                      <div className="mt-4 overflow-x-auto">
                        <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                          <thead>
                            <tr className="border-b border-slate-200 text-xs uppercase tracking-[0.2em] text-slate-500 dark:border-slate-800 dark:text-slate-400">
                              <th className="py-3 pr-4">{lang === 'th' ? 'วันเวลาแจ้งเตือน' : 'Alert time'}</th>
                              <th className="py-3 pr-4">{lang === 'th' ? 'รถ' : 'Vehicle'}</th>
                              <th className="py-3 pr-4">{lang === 'th' ? 'คนขับ' : 'Driver'}</th>
                              <th className="py-3">{lang === 'th' ? 'วิดีโอ' : 'Video'}</th>
                            </tr>
                          </thead>
                          <tbody>
                            {table.rows.map((row) => (
                              <tr
                                key={`${table.remarkType}-${row.id}`}
