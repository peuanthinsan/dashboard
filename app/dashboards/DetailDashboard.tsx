...
export default function DetailDashboard({
  ...
  const latestRemarkVideoSamples = useMemo<RemarkVideoGroup[]>(() => {
            ...
            <section className={dashboardSectionClass}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="text-sm text-slate-500 dark:text-slate-400">
                  {latestRemarkVideoSamples.length} {lang === 'th' ? 'ประเภทหมายเหตุ' : 'alert types'}
                </span>
              </div>
              {latestRemarkVideoSamples.length === 0 ? (
                ...
                <div className="mt-6 space-y-6">
                  {latestRemarkVideoSamples.map((sample) => (
                    ...
                    >
                      <h3 className="text-base font-semibold text-slate-900 dark:text-white">{sample.remarkType}</h3>
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {sample.rows.length} {lang === 'th' ? 'วิดีโอล่าสุด' : 'latest videos'}
                      </p>
                      <div className="mt-4 overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
                        <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                          <thead className="bg-slate-200/40 dark:bg-slate-900/50">
                            <tr className="text-left text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">
                              <th className="px-3 py-2">#</th>
                              <th className="px-3 py-2">{lang === 'th' ? 'รถ' : 'Vehicle'}</th>
                              <th className="px-3 py-2">{lang === 'th' ? 'คนขับ' : 'Driver'}</th>
                              <th className="px-3 py-2">{lang === 'th' ? 'วันเวลาแจ้งเตือน' : 'Alert date time'}</th>
                              <th className="px-3 py-2">{lang === 'th' ? 'วิดีโอ' : 'Video'}</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                            {sample.rows.map((row, index) => (
                              <tr key={row.id}>
                                ...
                                <td className="whitespace-nowrap px-3 py-2 text-slate-700 dark:text-slate-200">{row.vehicle}</td>
                                <td className="whitespace-nowrap px-3 py-2 text-slate-700 dark:text-slate-200">{row.driver}</td>
                                <td className="whitespace-nowrap px-3 py-2 text-slate-700 dark:text-slate-200">{row.time}</td>
                                <td className="whitespace-nowrap px-3 py-2">
                                  <a
                                    href={row.videoUrl}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center rounded-md border border-indigo-300/60 bg-indigo-500/10 px-2 py-1 text-xs font-semibold text-indigo-300 hover:border-indigo-200 hover:text-indigo-200"
                                  >
