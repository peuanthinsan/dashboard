...
export default function SimpleDashboard({ dashboardName, sheetId, sheetGid }: DashboardProps) {
  ...
  const alertSummaryRows = useMemo(() => {
    ...
    rows.forEach((row) => {
      const dateValue = findValue(row, ['Alert Date Time', 'Track Time', 'Date']);
      const parsed = parseDate(dateValue);
      const dateKey = parsed ? toDayKey(parsed) : 'Unknown date';
      const vehicle = String(findValue(row, ['Vehicle No', 'Vehicle No TH']) ?? '—');
      const key = `${dateKey}-${vehicle}`;
      const alertType = String(findValue(row, ['Alert Type']) ?? '').trim();
      const normalizedAlert = normalizeLabel(alertType);
      ...
      if (normalizedAlert.includes('distraction')) {
        entry.distraction += 1;
      }
      if (normalizedAlert.includes('fatigue') || normalizedAlert.includes('drowsy')) {
        entry.fatigue += 1;
      }
      if (normalizedAlert.includes('yawn')) {
        entry.yawning += 1;
      }
      entry.total += 1;
          ...
          <>
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="text-sm text-slate-400">{rows.length} alerts</span>
              </div>
              <div className="relative mt-4">
                {trendData.length === 0 ? (
                  <p className="text-sm text-slate-400">No alert activity available yet.</p>
                ) : (
                  <svg
            ...
            <section className="rounded-2xl border border-slate-200 bg-white/95 p-6 text-slate-900 shadow-lg">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold">Alerts by vehicle and date</h2>
                  <p className="text-sm text-slate-500">
                    Tip: Sort your sheet by date or vehicle to refresh the order here.
                  </p>
                </div>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="text-left">
                      <th className="bg-blue-500/80 px-4 py-3 font-semibold text-white">Date</th>
                      <th className="bg-cyan-300 px-4 py-3 font-semibold text-slate-900">Vehicle Number</th>
                      <th className="bg-blue-300/80 px-4 py-3 text-right font-semibold text-slate-900">
                        Distraction
                      </th>
                      <th className="bg-amber-200 px-4 py-3 text-right font-semibold text-slate-900">Fatigue</th>
                      <th className="bg-emerald-200 px-4 py-3 text-right font-semibold text-slate-900">Yawning</th>
                      <th className="bg-rose-300 px-4 py-3 text-right font-semibold text-slate-900">Total</th>
                    </tr>
                  </thead>
                    ...
                    {alertSummaryRows.length === 0 ? (
                      ...
                      alertSummaryRows.map((row) => (
                        <tr key={row.id} className="bg-white">
                          <td className="px-4 py-3 font-semibold text-slate-800">{row.dateLabel}</td>
                          <td className="px-4 py-3 text-slate-700">{row.vehicle}</td>
                          <td className="bg-blue-50/80 px-4 py-3 text-right text-slate-700">
                            {row.distraction}
                          </td>
                          <td className="bg-amber-50 px-4 py-3 text-right text-slate-700">{row.fatigue}</td>
                          <td className="bg-emerald-50 px-4 py-3 text-right text-slate-700">
                            {row.yawning}
                          </td>
                          <td className="bg-rose-50 px-4 py-3 text-right font-semibold text-slate-700">
                            {row.total}
                          </td>
                        </tr>
                      ))
