    ...
    <>
      ...
      >
        <form action={formAction} className="grid gap-4">
          ...
          <label className={`flex flex-col gap-2 ${ADMIN_LABEL}`}>
            Company
            <select name="companyId" defaultValue={organization.companyId ?? ''} className={ADMIN_INPUT}>
...
export default function OrganizationsClient({
  ...
  const companyMap = useMemo(
    () => new Map(companies.map((company) => [company.id, company.name ?? 'Unassigned company'])),
    [companies],
  );
    ...
    <AdminSection>
                    ...
                    <OrganizationRow
                      ...
                      companyName={
                        organization.companyId
                          ? companyMap.get(organization.companyId) ?? 'Unassigned company'
                          : 'Unassigned company'
                      }
                    />
                  ))}
      ...
      >
        <form action={organizationCreateAction} className="grid gap-4">
          ...
          <label className={`flex flex-col gap-2 ${ADMIN_LABEL}`}>
            Company *
            <select name="companyId" className={ADMIN_INPUT}>
              <option value="">Select company</option>
              {companies.map((company) => (
                <option key={company.id} value={company.id}>
                  {company.name}
