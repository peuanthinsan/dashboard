...
import { getDashboardByPublicId, getOrganizationById, getOrganizationsByIds, getUser } from 'app/db';
...
export default async function DashboardPage({ params }: { params: { id: string } }) {
  ...
  let matchesOrganization = dashboard.organizationId === null;
  if (dashboard.organizationId !== null) {
    const userOrganizations = await getOrganizationsByIds(userOrganizationIds);
    const selectedOrganizationsForCompany = new Set(
      userOrganizations
        .filter((organization) => organization.companyId === dashboard.companyId)
        .map((organization) => organization.id),
    );
    matchesOrganization = selectedOrganizationsForCompany.has(dashboard.organizationId);
  }
