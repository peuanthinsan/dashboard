...
export default async function DashboardPage() {
  ...
  const dashboards =
    user.length > 0
      ...
      : [];
  return (
    <div className="min-h-screen bg-gradient-to-b from-cyan-100 via-fuchsia-50 to-amber-100 px-4 py-8 text-slate-900 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 dark:text-white sm:px-6 sm:py-10">
      <div className="mx-auto flex w-full max-w-[1252px] flex-col gap-8">
        <header className={`flex flex-wrap items-start justify-between gap-6 ${panelClass}`}>
          <div className="flex max-w-2xl flex-col gap-3">
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">คุณเข้าสู่ระบบด้วยบัญชี</p>
            <h1 className="text-2xl font-semibold sm:text-3xl">{session?.user?.email}</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              ตรวจสอบผลการดำเนินงาน วิเคราะห์แนวโน้มเชิงลึก และแชร์ข้อมูลล่าสุดกับทีมของคุณ
            </p>
          </div>
