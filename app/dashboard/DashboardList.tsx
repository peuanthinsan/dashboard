...
const badgeClass =
  'inline-flex items-center gap-2 rounded-full border border-slate-200/70 bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-slate-600 dark:border-slate-700/70 dark:bg-slate-800/80 dark:text-slate-200';
const cardClass =
...
const templateMeta = (template: string) => {
  const normalized = template.trim().toLowerCase();

  const templates: Record<string, { label: string; dotClass: string }> = {
    detail: { label: 'Detail', dotClass: 'bg-emerald-400 dark:bg-emerald-300' },
    simple: { label: 'Simple', dotClass: 'bg-emerald-400 dark:bg-emerald-300' },
    summary: { label: 'Summary', dotClass: 'bg-emerald-400 dark:bg-emerald-300' },
    video: { label: 'Video', dotClass: 'bg-emerald-400 dark:bg-emerald-300' },
  };
      ...
      {dashboards.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {dashboards.map((dashboard) => {
            const template = templateMeta(dashboard.template);
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className={badgeClass}>
                        <span className={`h-2 w-2 rounded-full ${template.dotClass}`} />
                        {template.label}
                      </div>
                    </div>
