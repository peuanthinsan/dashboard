  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold"><T k="yourDashboards" /></h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            <T k="dashboardListIntro" />
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <span className={pillClass}>{dashboardCount} <T k="total" /></span>
          <span className={pillClass}>{templateCount} <T k="templates" /></span>
        </div>
      </div>

      {!hasDashboards ? (
        <div className={emptyStateClass}>
          <p className="text-base font-semibold text-slate-700 dark:text-slate-200"><T k="noDashboardsYet" /></p>
          <p className="mt-2">
            <T k="askAdmin" />
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {dashboards.map((dashboard) => (
            <Link
              ...
              className={cardClass}
              aria-label={`${dashboard.name} - dashboard`}
            >
