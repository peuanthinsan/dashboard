...
const normalizeRemarkKey = (value: SheetRecordValue) => {
  if (typeof value !== 'string') return '';
  const normalized = value.trim().toLowerCase();
  return normalized.replace(/(?:[-\\s])?a2$/i, '');
};
const resolveRemark = (remark: SheetRecordValue, alertType: SheetRecordValue) => {
...
const formatMonthYearLabel = (value: string | Date | null) => {
  if (!value) return 'Unspecified';
...
type AlertRow = {
  ...
  monthKey: string;
  vehicle: string;
  driver: SheetRecordValue;
  alertType: string;
  speed: number | null;
  remarks: string | null;
  fleet: SheetRecordValue;
  videoUrl: SheetRecordValue;
};
...
export default function DetailDashboard({ name, sheetUrl }: DetailDashboardProps) {
  const theme = useTheme();
  const { columns, records, formattedRows, loading, error, lastUpdated, refresh } = useGoogleSheet({ sheetUrl });
  ...
  const alertRows = useMemo<AlertRow[]>(() => {
    ...
    return records
      .map((row, index) => {
        ...
        const resolvedDate = dateValue instanceof Date ? dateValue : new Date(String(dateValue ?? ''));
        ...
        const dateKey = resolvedDate.toISOString().slice(0, 10);
        const resolvedRemark = resolveRemark(remarksColumn ? row[remarksColumn.field] : null, alertType);
        const speedValue = speedColumn ? row[speedColumn.field] : null;
        const speed = typeof speedValue === 'number' ? speedValue : speedValue ? Number(speedValue) : null;
        return {
          id: `${index}-${dateKey}`,
          dateKey,
          dateValue: resolvedDate,
          dateDisplay: formatDateLabel(resolvedDate),
          dateTimeDisplay: formattedRows[index]?.[dateTimeColumn.field] ?? formatDateTimeLabel(resolvedDate),
          monthKey: dateKey.slice(0, 7),
          vehicle: String(vehicle),
          driver: driverColumn ? row[driverColumn.field] : null,
          alertType: String(alertType),
          speed: Number.isNaN(speed ?? 0) ? null : speed,
  ...
  const filteredAlerts = useMemo(() => {
    ...
    if (selectedFleet !== 'all') {
      filtered = filtered.filter((row) => String(row.fleet ?? '') === selectedFleet);
    }
  ...
  const remarkFleetSummary = useMemo(() => {
    ...
    const legend = (theme.palette as { appEngineLegend?: Record<string, string> }).appEngineLegend || {};
    const remarkColors = [
      legend.purple || theme.palette.secondary.main,
      legend.teal || theme.palette.info.main,
      legend.green || theme.palette.success.main,
      legend.orange || theme.palette.warning.main,
      legend.blue || theme.palette.primary.main,
      legend.red || theme.palette.error.main,
    ];
  ...
  const monthOverMonthSummary = useMemo(() => {
    ...
    alertRows.forEach((row) => {
      ...
      if (selectedFleet !== 'all' && String(row.fleet ?? '') !== selectedFleet) {
        return;
      }
  ...
  const alertsSortComparator = useMemo(
    () =>
      buildMultiSortComparator(alertsSorts, {
        ...
        driver: (row: AlertRow) => (row.driver ? String(row.driver) : ''),
        alert: (row: AlertRow) => row.alertType,
        speed: (row: AlertRow) => row.speed ?? 0,
        remarks: (row: AlertRow) => row.remarks || '',
        video: (row: AlertRow) => (row.videoUrl ? String(row.videoUrl) : ''),
      }),
    [alertsSorts],
  );
  ...
  const handleAlertsSort = (columnId: string) => (event: React.MouseEvent) => {
  ...
  const columnVisualStyles = useMemo(() => {
    ...
    const legend = (palette as { appEngineLegend?: Record<string, string> }).appEngineLegend || {};
    return {
      Date: {
        header: {
      ...
      {loading ? (
        ...
        <Box sx={{ mt: 4 }}>
          {!vehicleColumn || !alertTypeColumn || !dateTimeColumn ? (
            <Alert severity="info">
              Add columns labelled &quot;Vehicle No&quot;, &quot;Alert Type&quot;, and a date/time column to view alert
              trends.
            </Alert>
          ) : (
            <>
                          ...
                          <TableHead>
                            <TableRow>
                              <TableCell
                                ...
                                sortDirection={getAlertsSortDirection('dateTime')}
                              >
                                <TableSortLabel
                              ...
                              <TableCell
                                ...
                                sortDirection={getAlertsSortDirection('vehicle')}
                              >
                                <TableSortLabel
                              ...
                              <TableCell sx={{ fontWeight: 700 }} sortDirection={getAlertsSortDirection('driver')}>
                                <TableSortLabel
                              ...
                              <TableCell
                                ...
                                sortDirection={getAlertsSortDirection('alert')}
                              >
                                <TableSortLabel
                              ...
                              <TableCell
                                ...
                                sortDirection={getAlertsSortDirection('speed')}
                              >
                                <TableSortLabel
                              ...
                              <TableCell sx={{ fontWeight: 700 }} sortDirection={getAlertsSortDirection('remarks')}>
                                <TableSortLabel
                              ...
                              <TableCell sx={{ fontWeight: 700 }} sortDirection={getAlertsSortDirection('video')}>
                                <TableSortLabel
